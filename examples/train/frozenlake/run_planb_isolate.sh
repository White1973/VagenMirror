...
# Silence wandb's verbose background-sync output (repeated "uploading
# wandb-metadata.json / requirements.txt / output.log ..." lines that flood the
# console when output.log grows fast on short-horizon envs like FrozenLake).
# Does NOT change training: wandb still records everything; only the print is
# suppressed. Set WANDB_MODE=offline additionally if uploads ever block
# training (then `wandb sync <run_dir>` after the run).
export WANDB_SILENT=true

PROJECT_NAME="jiayu_agent"
EXPERIMENT_NAME="ppo_qwen25vl3b_frozenlake_planb_isolate_0625"
...
REF_MODEL_PATH=/personal/jiayu2026/models/vagen_frozenlake
mkdir -p ${EXPERIMENT_DIR}

RAY_PORT=0 PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=5,6 /opt/conda/envs/vagen/bin/python3 -m vagen.main_ppo \
    ...
    custom_reward_function.reward_kwargs.lambda_cot=0.1 \
    ...
    custom_reward_function.reward_kwargs.verifier.mode=grounded \
