...
class DINOScoreCalculator(BaseMetric):
    def __init__(self, config=None, model_size='large', device='cuda:0'):
        ...
        # Normalize device: accept int (e.g. 0 -> "cuda:0"), str ("cuda:0"/"cpu"),
        # or None. Falls back to CPU when CUDA is unavailable.
        if device is None:
            device = "cpu"
    ...
    def get_DINOv2_model(self, model_size):
        ...
        # Allow network download to populate the HF cache when the local cache is
        # incomplete (the original `local_files_only=True` hard-failed if any weight
        # file was missing). Set VAGEN_DINO_LOCAL_ONLY=1 to force offline mode after
        # pre-caching the weights, which is useful for distributed training where
        # many env workers would otherwise race to download concurrently.
        local_only = os.environ.get("VAGEN_DINO_LOCAL_ONLY", "0") == "1"
        return (
            AutoModel.from_pretrained(model_size, local_files_only=local_only),
            AutoImageProcessor.from_pretrained(model_size, local_files_only=local_only),
        )
