...
def _find_hf_cache_parquet(dataset_name, split):
    """Find parquet files in HuggingFace hub cache (~/.cache/huggingface/hub/)."""
    hf_cache = os.path.expanduser("~/.cache/huggingface/hub")
    dataset_dir = os.path.join(hf_cache, f"datasets--{dataset_name.replace('/', '--')}")
    ...
    for snap in os.listdir(snapshots_dir):
        data_dir = os.path.join(snapshots_dir, snap, "data")
        if os.path.isdir(data_dir):
            parquet_file = os.path.join(data_dir, f"{split}-00000-of-00001.parquet")
...
def load_svg_dataset(data_dir, dataset_name, split):
    """Load the SVG dataset from local files or HuggingFace cache (offline-safe)."""
    # 1. Try local data_dir (saved_to_disk format)
    ...
    if os.path.exists(local_path):
        try:
            ...
            print(f"Error loading from local data_dir: {e}")

    # 2. Try reading directly from HF hub cache parquet files (no network needed)
    parquet_path = _find_hf_cache_parquet(dataset_name, split)
    if parquet_path:
        try:
            print(f"Loading dataset from HF cache parquet: {parquet_path}")
            dataset = load_dataset("parquet", data_files=parquet_path, split="train")
            return dataset
        except Exception as e:
            print(f"Error loading from HF cache parquet: {e}")

    # 3. Fallback: try load_dataset (may need network)
    try:
