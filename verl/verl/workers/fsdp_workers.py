...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # Reduce per-microbatch metric lists to scalars before cross-worker concat
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="red", role="actor_update_protected_multirole")
    def update_actor_protected_multirole(self, data: DataProto):
        """Run one protected optimizer step from isolated role gradients."""
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
        ...
        with self.ulysses_sharding_manager:
            data = data.to("cpu")
            with Timer(name="update_policy_protected_multirole", logger=None) as timer:
                metrics = self.actor.update_policy_protected_multirole(data=data)
