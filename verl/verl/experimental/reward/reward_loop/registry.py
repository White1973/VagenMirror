...
def get_reward_loop_manager_cls(name: str) -> type[RewardLoopManagerBase]:
    ...
    """
    # Backward compatibility: legacy PPO config commonly uses reward_manager=batch.
    # In experimental reward loop, `naive` is the equivalent single-sample manager.
    if name == "batch" and "naive" in REWARD_LOOP_MANAGER_REGISTRY:
        return REWARD_LOOP_MANAGER_REGISTRY["naive"]
